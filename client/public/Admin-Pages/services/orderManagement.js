...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function fetchOrders() {
    try {
      ...
      renderTable();
      // Reveal the page after data loads
      ...
      // On error, still reveal so user sees the message
      document.body.classList.remove('hidden');
    }
  }
  ...
  function getFiltered() {
    ...
    return orders.filter(o => {
      ...
      const stat     = o.status;
      const matchesSearch = [id, cust, date, totalStr].some(f => f.includes(q));
          ...
          <td>
            <select class="form-select form-select-sm status-select" data-id="${o._id}">
              ${['pending','processing','shipped','delivered','cancelled']
                .map(s => `<option value="${s}"${o.status===s?' selected':''}>
  ...
  function bindActions() {
    ...
    tbody.querySelectorAll('.issue-refund').forEach(btn =>
      btn.addEventListener('click', () =>
        confirm(`⚠️ Refund Order #${btn.dataset.id}?`) &&
  ...
  // 9) Logout handler (same as dashboard.js)

...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch(`/api/products/${productId}`)
    ...
    .then(async (product) => {
      ...
      let currentVariant =
        variants.find(
          (v) =>
            (selectedColorParam
              ? v.color.toLowerCase() === selectedColorParam.toLowerCase()
              : true) &&
      ...
      function render() {
        ...
        // Build color options (unchanged)
        const colorsHTML = allColors
          .map((color) => {
            const isActive =
              color.toLowerCase() === currentVariant.color.toLowerCase();
            return `
              <div 
        ...
        // Build sizes based on the selected color (unchanged)
        ...
        const sizesHTML = availableSizes
          .map((size) => {
        ...
        container.innerHTML = `
          ...
          <div class="col-md-6">
            ...
            ${(() => {
              // Use the product's base price with variant-specific discount
              const orig = product.price;
              const discountVal = currentVariant.discount || 0;
              if (discountVal > 0) {
                const disc = (orig * (1 - discountVal / 100)).toFixed(2);
                return `
                    <p>
                      <del class="text-muted">$${orig.toFixed(2)}</del>
                      <span class="text-danger h4">$${disc}</span>
                      <span class="badge bg-danger">-${discountVal}%</span>
                    </p>
                  `;
              } else {
      ...
      <div class="border-bottom py-2">
        <div>
          ...
          <small class="text-muted">by ${r.userName} on ${new Date(
                r.createdAt
    ...
    `
          ...
          reviewsContainer.innerHTML =
            '<p class="text-danger">Could not load reviews.</p>';
        }
      }
        ...
        try {
          ...
          let res = await fetch(
            `/api/products/related?category=${encodeURIComponent(
              product.category
            )}&brand=${encodeURIComponent(product.brand)}&exclude=${productId}`
          );
          ...
          if (relatedProducts.length < 3) {
            const resFallback = await fetch(
              `/api/products?exclude=${productId}`
            );
            const fallbackProducts = await resFallback.json();

            // Filter out duplicates (products already in relatedProducts)
            const additionalProducts = fallbackProducts.filter(
              (p) => !relatedProducts.some((r) => r._id === p._id)
            );
          ...
          suggestedContainer.innerHTML = relatedProducts
            .map(
              (related) => `
            <div class="col-6 col-md-3">
              <a href="product.html?id=${
                ...
                <div class="card h-100">
                  <img 
                    src="${
                      related.images[0] || "/assets/images/placeholder.jpg"
                    }" 
                    class="card-img-top" 
                    alt="${related.name}">
                  <div class="card-body text-center">
                    <h6 class="card-title mb-1">${related.name}</h6>
                    <p class="card-text text-danger fw-semibold">$${related.price.toFixed(
                      2
                    )}</p>
                  </div>
                </div>
          ...
          suggestedContainer.innerHTML =
            '<p class="text-danger">Could not load related products.</p>';
        }
      }
    ...
    .catch((err) => {
      ...
      container.innerHTML =
        '<p class="text-danger">Could not load product details.</p>';
    });
});
